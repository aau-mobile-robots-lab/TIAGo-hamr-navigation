...
for i = 1:length(robot_pose.Time(:))
    current(i) = robot_pose.Time(i) -robot_pose.Time(1);
end

N = 900;% Adjust the amount of time samples
zz1 = current(1:N);

figure(1)
surf([robot_pose.Data(1:N,1) robot_pose.Data(1:N,1)], ...
    [robot_pose.Data(1:N,2) robot_pose.Data(1:N,2)], ... 
    [ zz1(:) zz1(:)], ... 
...
hold on
double current_temp
for i = 1:length(MOPoses(:, 1))
    current_temp = MOobs_msg{i, 1}.Markers(4, 1).Header.Stamp.Sec - MOobs_msg{1, 1}.Markers(4, 1).Header.Stamp.Sec
    current1(i) = current_temp;
end
N1 = 80 % Adjust the amount of time samples
zz2 = current1(1:N1); %List of time until N1
plot(global_plan(:, 1), global_plan(:, 2), 'r', 'LineWidth', 1)
surf([MOPoses(1:N1, 4, 1) MOPoses(1:N1, 4, 1)], ...
    [MOPoses(1:N1, 4, 2) MOPoses(1:N1, 4, 2)], ... 
    [zz2(:) zz2(:)], ... 
    'FaceColor', 'none','EdgeColor', 'interp','LineWidth', 2);
hold on
plot(MOPoses(1, 4, 1),MOPoses(1, 4, 2),'c.','MarkerSize', 30); %Plot starting points for the robot and the moving obstacle 
plot(robot_pose.Data(1,1),robot_pose.Data(1,2),'m.','MarkerSize', 30);
...
legend('Robot Pose', 'Global plan','Moving Obstacles')
grid on
