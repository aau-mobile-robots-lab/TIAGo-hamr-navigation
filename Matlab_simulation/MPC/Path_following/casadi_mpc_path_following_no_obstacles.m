...
omega = SX.sym('omega');
ddp = SX.sym('ddp');
controls = [v;omega;ddp];
n_controls = length(controls);

rhs = [v*cos(theta);v*sin(theta);omega;dp;ddp];     %right hand side of the system
...
P = SX.sym('P',n_states ...                     % Initial states (x0)
               + N*(n_states+n_controls));            % MO states in each prediction
...
clf
Simulate_MPC_path_following_no_obstacles (x_ol,x_cl,x_ref,N,rob_diameter)
