...
% Obstacle params
n_obstacle = 3;     % number of obstacles
O_init = [3.0, 1.0, pi/2, 0.5, 0.3;
         2.0, 3.5, 0.0, 0.5, 0.3;
         3.5, 1.5, pi, 0.7, 0.2];      %Obstacle X, Y, Theta, velocity, radius
...
% In reality we dont need this... but lets just keep it
Ox = SX.sym('Ox');
Oy = SX.sym('Oy');
Oth = SX.sym('Oth');
Ov = SX.sym('Ov');
...
P = SX.sym('P',n_states + n_states + n_obstacle*(N+1)*n_Ost);
...
for k = 1:N+1      
    for i = 1:n_obstacle
        %parameters in P stored in a complicated way
        % but i_pos this magic position shifting works in paper for now.
...
while(norm((x0-x_goal),2) > goal_tolerance && mpc_i < sim_time / Ts)
    ...
    for k = 1:N+1
        for i = 1:n_obstacle
            ...
            args.p(i_pos+2:i_pos+4) = [O_init(i,3), O_init(i,4), O_init(i,5)];
            o_cl(i,k,3:5,mpc_i+1) = [O_init(i,3), O_init(i,4), O_init(i,5)];
            
            t_predicted = (mpc_i + k-1)*Ts;     % Time at predicted state
            
            obs_x = O_init(i,1)+t_predicted*O_init(i,4)*cos(O_init(i,3));
            obs_y = O_init(i,2)+t_predicted*O_init(i,4)*sin(O_init(i,3));
...
clf
Simulate_MPC_multiple_varying_obs (x_ol,x_cl,o_cl,x_goal,N,rob_diameter)
