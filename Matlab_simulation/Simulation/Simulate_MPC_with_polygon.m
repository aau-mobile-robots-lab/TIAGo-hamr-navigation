function Simulate_MPC_with_polygon (x_ol,x_cl,o_cl,SO_dims,SO_poses,x_ref,N,rob_diameter)
%% Figure setup
figure(100)
...
for k = 1:step_size:size(x_ol,2)-1 % go through the open loop
    ...
    for i = 1:size(SO_dims,2)
        if SO_dims<=2
            plot(SO_poses(i_pos:i_pos+SO_dims(i)-1,1), SO_poses(i_pos:i_pos+SO_dims(i)-1,2), 'm-o');
            hold on;
            i_pos = i_pos+SO_dims(i);
        else
            poly_x = [SO_poses(i_pos:i_pos+SO_dims(i)-1,1);SO_poses(i_pos,1)];
            poly_y = [SO_poses(i_pos:i_pos+SO_dims(i)-1,2);SO_poses(i_pos,2)];
